...
class xcx():
    ...
    def setData(self):
        op = 'sed -i "" "s/appId:\'\'/appId:\'%s\'/;' \
             ...
             % (self.id, self.name, self.appDescription, self.imageurl," "+os.path.join(self.appPath,self.id,"XcxShopApp","app.js"))

        os.system(op)

    def createZip(self):
        os.system("zip -rjq %s.zip %s &&mv %s.zip %s" % (self.id,os.path.join(self.appPath,self.id),self.id,self.appPath ))
